...
biomarkers = pd.read_csv('results/generate_SBBs/' + study  + 
...
taxa_vs_variants = pd.read_csv('data/' + study + '/' + str(msa_method) + '_taxa_vs_variants_unique.tsv', sep='\t')
...
print('here')
person_variant = np.array(np.load('results/generate_SBBs/' + study  + 
                                  '_'  + str(msa_method) +  '_person_variant' + str(order) + '_unique.npy'))
print('here1')
